  ...
  return (
    <Box>
      <PageHeader
        ...
        breadcrumbs={[{ label: 'Teacher' }, { label: 'Schedule' }]}
      />

      {teacherSchedules.length === 0 ? (
        <Paper variant="outlined" sx={{ p: 4, textAlign: 'center' }}>
          <Typography color="text.secondary">No schedule assigned yet. Please contact the principal.</Typography>
        </Paper>
      ) : (
        <TableContainer component={Paper} variant="outlined">
          <Table>
            ...
            <TableBody>
              {DAYS.flatMap(day => {
                const daySchedules = teacherSchedules.filter(s => s.dayOfWeek.toUpperCase() === day);
