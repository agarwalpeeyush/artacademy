...
type EditableFields = Pick<Teacher, 'firstName' | 'lastName' | 'email' | 'phone' | 'qualification'>;
  ...
  return (
    <Box>
      <PageHeader
        ...
        breadcrumbs={[{ label: 'Teacher' }, { label: 'Profile' }]}
        action={
          editing ? (
            ...
            <CardContent sx={{ textAlign: 'center', p: 4 }}>
              ...
              >
                <Typography variant="h3" color="white">
                  {(profile?.firstName?.[0] ?? user?.username?.[0] ?? 'T').toUpperCase()}
                </Typography>
              </Box>
              ...
              <Chip
                label={profile?.status === 'ACTIVE' ? 'Active Teacher' : (profile?.status || 'Unknown')}
