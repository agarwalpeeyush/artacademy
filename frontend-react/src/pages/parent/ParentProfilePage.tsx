...
type EditableFields = Pick<Parent, 'firstName' | 'lastName' | 'email' | 'phone' | 'address' | 'occupation' | 'relationship'>;
...
const ParentProfilePage: React.FC = () => {
  ...
  const handleSave = async () => {
    ...
    try {
      setSaving(true);
      const updated = await parentService.updateMyProfile(form);
      setProfile(updated);
      setEditing(false);
      setForm(null);
  ...
  return (
    <Box>
      <PageHeader
        ...
        breadcrumbs={[{ label: 'Parent' }, { label: 'Profile' }]}
        action={
          editing ? (
            ...
            <CardContent sx={{ textAlign: 'center', p: 4 }}>
              ...
              >
                <Typography variant="h3" color="white">
                  {(profile?.firstName?.[0] ?? user?.username?.[0] ?? 'P').toUpperCase()}
                </Typography>
              </Box>
              ...
              <Chip
                label={profile?.status === 'ACTIVE' ? 'Active' : (profile?.status || 'Unknown')}
              ...
              <Grid container spacing={2}>
                ...
                <Grid item xs={12} sm={6}>
                  {editing
                    ...
                    : <ProfileField label="Relationship" value={profile?.relationship} />}
                </Grid>
                <Grid item xs={12}>
                  {editing
