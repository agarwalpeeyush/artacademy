...
const StudentSchedulePage: React.FC = () => {
  ...
  const { studentSchedules, loading } = useSelector((state: RootState) => state.schedules);
  const today = new Date().toLocaleDateString('en-US', { weekday: 'long' }).toUpperCase();

  useEffect(() => {
    if (user?.id) dispatch(fetchStudentSchedules(user.id));
  }, [dispatch, user]);
    ...
    <Box>
      ...
      {studentSchedules.length === 0 ? (
        <Paper variant="outlined" sx={{ p: 4, textAlign: 'center' }}>
          <Typography color="text.secondary">No schedule available. Please contact administration.</Typography>
        </Paper>
      ) : (
        <TableContainer component={Paper} variant="outlined">
          <Table>
            ...
            <TableBody>
              {DAYS.flatMap(day => {
                const daySchedules = studentSchedules.filter(s => s.dayOfWeek.toUpperCase() === day);
