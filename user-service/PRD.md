...
| PPL-13 | Create a parent linked to a student; publish `parent-created` on first login | PRINCIPAL | `POST /parents` |
| PPL-14 | Link an existing parent login to an additional child (no new auth event) | PRINCIPAL | `POST /parents` (same `loginId`) |
...
| PPL-17 | Parent updates own contact details across all linked rows | (self via PUT rule) | `PUT /parents/me` |
...
2. **User Service does not own credentials** — passwords and roles live in
   auth-service. The `*-created` event carries a `temporaryPassword` (defaulting to
   `Welcome@123` when the UI omits it) and a role list for provisioning.
3. **Unique login ID across all user types** — enforced by the shared `USERS.LOGIN_ID`
   unique constraint and `existsByLoginId`. `loginId` equals the auth username.
...
6. **Parent multi-child model** — one parent login may have multiple `PARENTS` rows
   (one per child). The auth event is published only for the first row of a login;
   further rows add child links only.
7. **Parent must reference an existing student** — `POST /parents` fails with
   not-found if `studentId` does not exist.
...
11. **Self-service is scoped** — `/me` updates only change safe contact fields, never
    identity keys (`employeeCode`, `status`, links).
